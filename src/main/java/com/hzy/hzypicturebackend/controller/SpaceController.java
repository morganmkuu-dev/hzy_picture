...
public class SpaceController {
    ...
    public BaseResponse<Boolean> deleteSpace(@RequestBody DeleteRequest deleteRequest, HttpServletRequest request) {
        ...
        // 操作数据库：改为调用刚刚在 Service 层写好的带事务的级联删除方法
        boolean result = spaceService.deleteSpaceAndPictures(id);
     ...
     * 管理员发放扩容包
     */
    @PostMapping("/add_resource_pack")
    @AuthCheck(mustRole = UserConstant.ADMIN_ROLE) // 仅限管理员调用
    public BaseResponse<Boolean> addResourcePack(@RequestBody SpaceResourcePackAddRequest request,
                                                 HttpServletRequest httpServletRequest) {
        if (request == null) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR);
        }
        User loginUser = userService.getLoginUser(httpServletRequest);
        boolean result = spaceService.addResourcePack(request, loginUser);
    ...
    @AuthCheck(mustRole = UserConstant.ADMIN_ROLE)
    public BaseResponse<String> checkAndFixSpaceQuota(HttpServletRequest request) {
        spaceService.checkAndFixSpaceQuota();
        return ResultUtils.success("历史空间额度校准任务执行成功，请查看后端日志详情");
    }
    /**
     * 返回空间列表
     * @return
